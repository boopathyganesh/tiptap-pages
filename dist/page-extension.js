...
import { DEFAULT_PAGE_OPTIONS } from './types';
...
import { pageFormatToOptions, PAGE_FORMATS } from './page-formats';
import { BULLETLIST, CITATION, HARDBREAK, HEADING, LISTITEM, ORDEREDLIST, PARAGRAPH, RECORDING_LOADER, TEMPLATE_VARIABLE, TRANSIENT_TEXT, } from './node-names';
...
function deepMerge(defaults, userOptions) {
    ...
    if (userOptions.bodyHeight !== undefined)
        result.bodyHeight = userOptions.bodyHeight;
    if (userOptions.bodyWidth !== undefined)
        result.bodyWidth = userOptions.bodyWidth;
    if (userOptions.bodyPadding !== undefined)
        result.bodyPadding = userOptions.bodyPadding;
    if (userOptions.headerHeight !== undefined)
        result.headerHeight = userOptions.headerHeight;
    if (userOptions.footerHeight !== undefined)
        result.footerHeight = userOptions.footerHeight;
    if (userOptions.types !== undefined)
        result.types = userOptions.types;
    if (userOptions.headerData !== undefined)
        result.headerData = userOptions.headerData;
    if (userOptions.footerData !== undefined)
        result.footerData = userOptions.footerData;
    return result;
}
export const PageExtension = Extension.create({
    ...
    onBeforeCreate() {
        ...
        if (bodyHeight === undefined || bodyHeight === null) {
            throw new Error('PageExtension: bodyHeight is required but not provided. ' +
                'Please provide a numeric value for bodyHeight in PageExtension.configure().');
        }
        if (bodyWidth === undefined || bodyWidth === null) {
            throw new Error('PageExtension: bodyWidth is required but not provided. ' +
                'Please provide a numeric value for bodyWidth in PageExtension.configure().');
        }
        if (typeof bodyHeight !== 'number' || bodyHeight <= 0) {
            throw new Error(`PageExtension: bodyHeight must be a positive number, but got ${bodyHeight}. ` +
                'Please provide a valid numeric value greater than 0.');
        }
        if (typeof bodyWidth !== 'number' || bodyWidth <= 0) {
            throw new Error(`PageExtension: bodyWidth must be a positive number, but got ${bodyWidth}. ` +
                'Please provide a valid numeric value greater than 0.');
        }
        this.options = deepMerge(DEFAULT_PAGE_OPTIONS, this.options);
